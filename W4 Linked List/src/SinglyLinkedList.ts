...
class SinglyNode {
    data: any; // ข้อมูล
    next: SinglyNode | null; // ตัวแปรชี้ไปยัง Node ถัดไป
    constructor(data: any) { // กำหนดค่าเริ่มต้นให้กับ Node
...
class SinglyLinkedList { // สร้าง Class SinglyLinkedList
    head: SinglyNode | null; // ตัวแปรชี้ไปยัง Node แรกของ Linked List
    tail: SinglyNode | null; // ตัวแปรชี้ไปยัง Node สุดท้ายของ Linked List
    length: number = 0; // ตัวแปรเก็บค่าของจำนวน Node ทั้งหมดใน Linked List
    constructor() { // กำหนดค่าเริ่มต้นให้กับ Linked List
        this.head = null; // กำหนดค่าให้กับตัวแปร head เป็น null
        this.tail = null; // กำหนดค่าให้กับตัวแปร tail เป็น null
    }

    append(value: any) { // สร้าง Method append ใน Class SinglyLinkedList
    ...
    appendLeft(value: any) { // สร้าง Method appendLeft ใน Class SinglyLinkedList
        const newNode = new SinglyNode(value) // สร้าง Node ใหม่
        if (this.head === null) { // ถ้า Linked List ยังไม่มี Node ใดๆ
            this.head = newNode // ให้ Head และ Tail ชี้ไปที่ Node ใหม่
            this.tail = this.head // ให้ Head และ Tail ชี้ไปที่ Node ใหม่
    ...
    toArray() {
        const array: any[] = [] // สร้างตัวแปร array เป็น Array เปล่า
        ...
        while (node !== null) { // วนลูปเพื่อเก็บค่าของ Node ทุกตัวใน Linked List ไว้ในอาร์เรย์
            array.push(node?.data) // เพิ่มค่าของ Node ลงในอาร์เรย์  เครื่องหมาย ? คือ ถ้า node ไม่ใช่ null ให้เพิ่มค่าของ Node ลงในอาร์เรย์
            node = node?.next // ให้ตัวแปร node ชี้ไปยัง Node ถัดไป
        }
        return array // ส่งค่าอาร์เรย์กลับ
    }

    traverseToIndex(index: number) { // สร้าง Method traverseToIndex ใน 
    ...
    remove(index : number) { // สร้าง Method remove ใน Class SinglyLinkedList
        ...
        if (index < 1) { // ถ้า index น้อยกว่า 1
            this.head = this.head?.next // ให้ Head ชี้ไปยัง Node ถัดไป
