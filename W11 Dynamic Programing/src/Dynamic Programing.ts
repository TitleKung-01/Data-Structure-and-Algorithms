...
function dynamicFibonacci() { // สร้างฟังก์ชัน dynamicFibonacci
    interface NumberArray { // สร้าง interface NumberArray
        [index: number]: number // กำหนด index เป็น number และคืนค่าเป็น number
    } // ปิด interface NumberArray
    let cache: NumberArray = {} // สร้างตัวแปร cache และกำหนดค่าเป็น {}
    return function fibonacci(n: number): number { // สร้างฟังก์ชัน fibonacci ที่รับค่า n เป็น number และคืนค่าเป็น number
...
let countRecursive = 0 // สร้างตัวปร countRecursive และกำหนดค่าเป็น 0
function recursiveFibonacci(n: number): number { // สร้างฟังก์ชัน recursiveFibonacci ที่รับค่า n เป็น number และคืนค่าเป็น number
...
function iterativeFibonacci(n: number): number {    // สร้างฟังก์ชัน iterativeFibonacci ที่รับค่า n เป็น number และคืนค่าเป็น number
